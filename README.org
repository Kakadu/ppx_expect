...
This is a fork of https://github.com/janestreet/ppx_expect with lesser dependecies.
It should be used as drop-in preplacement of ppx_expect

Changes:

* time_now is replaced by mtime
* These dependecies are removed
  * base
  * ppx_compare
  * ppx_here
* Tests are removed, because they looked like seriously broken
* Js_of_ocaml support is removed (for simplicity)
...
configuration module =Expect_test_nobase_config=. The default module in scope
...
In addition to =Async.Expect_test_nobase_config=, there is an
alternative, =Async.Expect_test_nobase_config_with_unit_expect=.  That is
easier to use than =Async.Expect_test_nobase_config= because =[%expect]= has
...
=Expect_test_nobase_config_with_unit_expect= arrived in 2019-06.  We hope to
transition from =Expect_test_nobase_config= to
=Expect_test_nobase_config_with_unit_expect=, eventually renaming the latter
as the former.
...
module Expect_test_nobase_config :
  Expect_test_nobase_config_types.S
