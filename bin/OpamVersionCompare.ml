...
let rec skip_while_from i f w m =
  if (i : int) = m then i else if f w.[i] then skip_while_from (i + 1) f w m else i
;;
...
let compare_chunks x y =
  ...
  and compare_numerical xi yi =
    ...
    and yn =
      skip_while_from yi is_digit y yl
      (* length of numerical part *)
    in
    let comp = Int.compare (xn - xi) (yn - yi) in
    if comp = 0
...
let compare (x : string) (y : string) =
  ...
  then 0
    ...
    if u_comp <> 0
    then normalize_comp_result u_comp
    else normalize_comp_result (compare_chunks r1 r2))
;;

let equal (x : string) (y : string) = if (x : string) = y then true else compare x y = 0
